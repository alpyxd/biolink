...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS security_logs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    event_type TEXT NOT NULL,       -- 'failed_login', 'successful_login', 'rate_limited'
...
const defaultSettings = {
  ...
  // Social & Custom Link Appearance
  link_color_mode: 'original', // 'original' (logo brand colors) or 'custom' (single chosen color)
  link_custom_color: '#ffffff',
  link_glow_enabled: '1'
};
...
// Security: In-Memory Login Rate Limiter & Brute-force Shield
const loginAttempts = new Map();

function checkLoginRateLimit(ip) {
  const now = Date.now();
  const record = loginAttempts.get(ip) || { count: 0, resetAt: now + 15 * 60 * 1000, lockedUntil: 0 };

  if (record.lockedUntil > now) {
    const waitMins = Math.ceil((record.lockedUntil - now) / 60000);
    return { allowed: false, error: `Çok fazla hatalı deneme! Güvenlik nedeniyle ${waitMins} dakika kilitlendi.` };
  }
...
function recordFailedLogin(ip) {
  ...
  record.count += 1;
  if (record.count >= 5) {
    record.lockedUntil = now + 15 * 60 * 1000; // 15-minute lock after 5 fails
  }
...
function logSecurityEvent(eventType, username, ip, userAgent, details) {
  try {
    db.prepare(`
      INSERT INTO security_logs (event_type, username_attempted, ip_address, user_agent, details, created_at)
      VALUES (?, ?, ?, ?, ?, unixepoch())
    `).run(
      eventType,
...
// Security: Safe Path Traversal Validator
function isPathInside(childPath, parentDir) {
  const rel = path.relative(path.resolve(parentDir), path.resolve(childPath));
  return !rel.startsWith('..') && !path.isAbsolute(rel);
}

// Security: URL Scheme Sanitizer (Blocks dangerous javascript:, data:, vbscript:)
function sanitizeUrl(urlStr) {
  if (!urlStr || typeof urlStr !== 'string') return '';
  const trimmed = urlStr.trim();
  const dangerousRegex = /^(javascript|data|vbscript|file):/i;
  if (dangerousRegex.test(trimmed)) {
    return 'https://#invalid-link';
  }
  return trimmed;
}
...
// Request Body Reader with configurable Max Bytes (Memory Exhaustion / DoS Shield)
function readBody(req, maxBytes = 2 * 1024 * 1024) {
  return new Promise((resolve, reject) => {
    ...
    req.on('data', chunk => {
      ...
      if (size > maxBytes) {
        req.destroy();
        const err = new Error('İstek boyutu çok büyük (Payload Too Large)');
        err.statusCode = 413;
        reject(err);
        return;
      }
...
// MIME Types Map (Secure: Excludes raw SVG to prevent stored XSS attacks)
const MIME_TYPES = {
...
// Static file server with range request support and Security Headers
...
// 404 Not Found Page Server
function serve404(req, res) {
  const notFoundHtmlPath = path.join(PUBLIC_DIR, '404.html');
  if (fs.existsSync(notFoundHtmlPath)) {
    res.writeHead(404, {
      'Content-Type': 'text/html; charset=utf-8',
      ...SECURITY_HEADERS
    });
    return fs.createReadStream(notFoundHtmlPath).pipe(res);
  }
  res.writeHead(404, {
...
// JSON responder with Security Headers
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === '/api/profile' && method === 'GET') {
      ...
      // Filter out admin sessions & preview iframes from visitor tracking
      const isAdmin = verifyAuth(req);
      const isPreview = req.headers['sec-fetch-dest'] === 'iframe' || 
                        req.headers['referer']?.includes('/admin') || 
                        req.headers['referer']?.includes('/alpy');
    ...
    // POST /api/analytics/click -> Records link click with smart deduplication & admin exclusion
    if (pathname === '/api/analytics/click' && method === 'POST') {
      ...
      const isAdmin = verifyAuth(req);
      const isPreview = req.headers['referer']?.includes('/admin') || req.headers['referer']?.includes('/alpy');

      if (linkId && !isAdmin && !isPreview) {
        const ip = getClientIp(req);
        const ipHash = hashIp(ip);
        const fifteenMinsAgo = Math.floor(Date.now() / 1000) - (15 * 60);

        // Deduplicate: Don't count multiple clicks on the same link from same IP within 15 minutes
        const recentClick = db.prepare(`
          SELECT id FROM analytics
          WHERE ip_hash = ? AND event_type = 'link_click' AND target_id = ? AND created_at > ?
        `).get(ipHash, linkId, fifteenMinsAgo);
    ...
    // POST /api/auth/login -> Rate-limited and brute-force protected with Audit Logging
    if (pathname === '/api/auth/login' && method === 'POST') {
      const clientIp = getClientIp(req);
      const userAgent = req.headers['user-agent'] || '';
      const rateCheck = checkLoginRateLimit(clientIp);

      const bodyRaw = await readBody(req, 64 * 1024);
      let payload = {};
      try { payload = JSON.parse(bodyRaw.toString()); } catch {}
      ...
      if (!rateCheck.allowed) {
        logSecurityEvent('rate_limited', username, clientIp, userAgent, 'Rate limit aşıldı (15 dk kilit)');
        return sendJSON(res, { error: rateCheck.error }, 429);
      }
      ...
      if (!admin || !verifyPassword(password, admin.password_hash, admin.salt)) {
        ...
        const remaining = 5 - ((loginAttempts.get(clientIp)?.count) || 1);
        const warnMsg = remaining > 0 
          ? `Geçersiz kullanıcı adı veya şifre! (Kalan deneme hakkı: ${remaining})`
          : 'Çok fazla hatalı deneme! 15 dakika kilitlendi.';
        
        logSecurityEvent('failed_login', username, clientIp, userAgent, remaining > 0 ? 'Hatalı şifre veya kullanıcı adı' : '5 kez hatalı deneme (Kilitlendi)');
        return sendJSON(res, { error: warnMsg }, 401);
      }

      recordSuccessLogin(clientIp);
      logSecurityEvent('successful_login', username, clientIp, userAgent, 'Başarılı admin girişi');
    ...
    if (pathname.startsWith('/api/admin/')) {
      ...
      // POST /api/admin/links -> Create link (URL Sanitized)
      if (pathname === '/api/admin/links' && method === 'POST') {
        const bodyRaw = await readBody(req, 128 * 1024);
        let payload = {};
        try { payload = JSON.parse(bodyRaw.toString()); } catch {}
      ...
      // PUT /api/admin/links/:id -> Update link (URL Sanitized)
      ...
      // POST /api/admin/upload -> Hardened file uploader (Max 25MB, Safe Ext only, SVG excluded)
      if (pathname === '/api/admin/upload' && method === 'POST') {
        ...
        const bodyBuffer = await readBody(req, 25 * 1024 * 1024); // Max 25MB
        ...
        const fileName = `${Date.now()}-${crypto.randomBytes(8).toString('hex')}${rawExt}`;
        const filePath = path.resolve(UPLOADS_DIR, fileName);

        if (!isPathInside(filePath, UPLOADS_DIR)) {
          return sendJSON(res, { error: 'Geçersiz dosya konumu.' }, 403);
        }
        ...
        return sendJSON(res, {
          ...
          original_name: path.basename(filePart.filename),
      ...
      // GET /api/admin/security/logs -> Returns latest security audit logs & IP attempts
      if (pathname === '/api/admin/security/logs' && method === 'GET') {
        const logs = db.prepare(`
          SELECT id, event_type, username_attempted, ip_address, user_agent, details, created_at
          FROM security_logs
          ORDER BY created_at DESC, id DESC
          LIMIT 100
        `).all();

        const totalFailed = db.prepare(`SELECT COUNT(*) as cnt FROM security_logs WHERE event_type = 'failed_login' OR event_type = 'rate_limited'`).get().cnt;
        const totalSuccessful = db.prepare(`SELECT COUNT(*) as cnt FROM security_logs WHERE event_type = 'successful_login'`).get().cnt;
        const now = Date.now();
        const currentlyLocked = Array.from(loginAttempts.values()).filter(r => r.lockedUntil > now).length;
      ...
      // POST /api/admin/security/logs/clear -> Clear security audit logs
      if (pathname === '/api/admin/security/logs/clear' && method === 'POST') {
        db.prepare('DELETE FROM security_logs').run();
        return sendJSON(res, { success: true, message: 'Güvenlik günlüğü başarıyla temizlendi.' });
      }
    ...
    // ==========================================
    // 📁 STATIC FILES & ROUTING (404 Error Shield)
    // ==========================================

    // 1. Root biolink public page
    if (pathname === '/' || pathname === '/index.html') {
      return serveStatic(req, res, path.join(PUBLIC_DIR, 'index.html'));
    }

    // 2. Admin dashboard page (EXCLUSIVELY /alpy)
    if (pathname === '/alpy' || pathname === '/alpy/') {
      return serveStatic(req, res, path.join(PUBLIC_DIR, 'admin.html'));
    }

    // 3. Uploaded media files: /uploads/*
    if (pathname.startsWith('/uploads/')) {
      const subPath = pathname.replace(/^\/uploads[\/\\]*/, '');
      const cleanRelativePath = subPath.replace(/^[\/\\]+/, '');
      const safePath = path.normalize(cleanRelativePath).replace(/^(\.\.[\/\\])+/, '');
      const filePath = path.join(UPLOADS_DIR, safePath);

      if (!isPathInside(filePath, UPLOADS_DIR) || !fs.existsSync(filePath) || !fs.statSync(filePath).isFile()) {
        return serve404(req, res);
      }
      return serveStatic(req, res, filePath);
    }

    // 4. Public frontend assets (CSS, JS, Fonts, Images)
    const cleanRelativePath = pathname.replace(/^[\/\\]+/, '');
    const safePublicPath = path.normalize(cleanRelativePath).replace(/^(\.\.[\/\\])+/, '');
    const filePath = path.join(PUBLIC_DIR, safePublicPath);

    if (isPathInside(filePath, PUBLIC_DIR) && fs.existsSync(filePath) && fs.statSync(filePath).isFile()) {
      return serveStatic(req, res, filePath);
    }

    // 5. Any other arbitrary path -> 404 Not Found Page!
    return serve404(req, res);
...
server.listen(PORT, () => {
  ...
  console.log(`⚙️  Admin Paneli   : http://localhost:${PORT}/alpy`);
